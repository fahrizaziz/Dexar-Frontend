...
export const MfeAdminGovernance: React.FC<Props> = ({ activeSubTab }) => {
  ...
  return (
    <div className="relative space-y-4">
      ...
      <div className="flex items-center justify-between bg-amber-500/5 border border-amber-500/20 px-4 py-2 rounded-2xl text-[11px] font-mono text-amber-400">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
          <span className="font-bold">MFE Remote: mfe-admin-governance (v1.8.5)</span>
        </div>
        <div className="hidden sm:flex items-center gap-4 text-zinc-500">
          <span className="flex items-center gap-1">
            <Wifi className="w-3 h-3 text-amber-500" /> Module Federation Active
          </span>
          <span className="flex items-center gap-1">
            <Cpu className="w-3 h-3 text-amber-500" /> Security & Governance Sandbox
          </span>
        </div>
      </div>
